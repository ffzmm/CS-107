...
static void test_sudoku()
{
    ...
    printf("%s\n", boolstr(is_single(1022,0,0)));
    printf("%s\n", boolstr(is_single(680, 6, 336)));
}

// The SFMT/UFMT is #defined to correct format for bitwidth setting of stype/utype
static void test_saturating()
{
    stype s1 = -90, s2 = -116, s3 = 90;
    utype u1 = 10, u2 = 200, u3 = 230 , u4 = 200;
    ...
    printf(UFMT" + "UFMT" = "UFMT" (unsigned)\n", u3, u4, sat_add_unsigned(u3, u4));
}

static void test_disassemble()
{
    // these constant arrays represented raw instruction bytes to be disassembled
    unsigned char imm[] =    {0x68, 0x10, 0x3f, 0x32, 0x00 }; // push 4-byte immediate
    unsigned char reg[] =    {0x53};                          // push register
    unsigned char ind[] =    {0xff, 0x32};                    // push register indirect
    unsigned char displ[] =  {0xff, 0x70, 0x48};              // push register indirect w/ displacement
    unsigned char scaled[] = {0xff, 0x74, 0x9d, 0xff};        // push register indirect w/ displacement & scaled index
